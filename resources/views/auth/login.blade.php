...
<body class="h-full flex items-center justify-end px-24 py-8">
    <div class="w-full max-w-lg md:w-1/2 lg:w-1/3 mr-16">
        <div class="text-center mb-6">
            <img class="mx-auto h-32 w-auto" src="images/logo_iclabs.png" alt="SiMonik Logo">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div>
                ...
                <div class="mt-1">
                    <input id="email" name="email" type="email" required autocomplete="email"
                        class="appearance-none block w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-white bg-opacity-75 text-gray-900 @error('email') border-red-500 @enderror"
                        value="{{ old('email') }}">
                    @error('email')
            ...
            <div>
                ...
                <div class="mt-1">
                    <input id="password" name="password" type="password" required autocomplete="current-password"
                        class="appearance-none block w-full px-3 py-2 border border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-white bg-opacity-75 text-gray-900 @error('password') border-red-500 @enderror">
                    @error('password')
                        <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <div class="text-sm">
                        <a href="{{ route('password.request') }}" class="font-medium text-gray-300 hover:text-blue-300">Lupa
                            password?</a>
                    </div>
                @endif
            </div>
